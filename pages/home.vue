<template>
    <div class="relative z-10 max-w-screen-sm">
      ...
      <div class="uQxNj" v-if="user">
        <NuxtLink to="/practice"><button class="ieMfVH" :disabled="loading">
    ...
    <NuxtLink to="/translation">
        <button  class="ieMfVH" :disabled="loading">
          <span class="fKlELC" :class="{loading: loading}">
            Translation
          </span>
          <svg viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" class="jjoFVh" :class="{loading: loading}">
    ...
    <NuxtLink to="/exam">
        <button @click="start" class="ieMfVH" :disabled="loading">
          <span class="fKlELC" :class="{loading: loading}">
            Exam
          </span>
          <svg viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" class="jjoFVh" :class="{loading: loading}">
  ...
  useHead({
    title: 'Home',
    meta: [
      { name: 'description', content: 'You can choose any of the following options.' }
    ]
  })
  </script>
